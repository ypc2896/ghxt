...
public class TongjiBizImpl implements TongjiBiz {
	...
	public String findTjfxRi(String wheres, List<String> dates) {
		...
		//String hql2 = "select DATE_FORMAT(t.riqi,'%Y-%m-%d'),sum(t.sfje),sum(t.cbje) from Tkd as t where t.jystate=1 "+wheres;
		 String hql =  "select to_char(t.riqi,'yyyy-mm-dd'),sum(t.sfje),sum(t.cbje) from Ckd t where t.jystate=1 "+wheres;
		 String hql2 = "select to_char(t.riqi,'yyyy-mm-dd'),sum(t.sfje),sum(t.cbje) from Tkd t where t.jystate=1 "+wheres;
	...
	public String findTjfxYue(String wheres, List<String> dates) {
		...
		String hql = "select to_char(t.riqi,'yyyy-mm'),sum(t.sfje),sum(t.cbje) from Ckd t where t.jystate=1 "+wheres;
		String hql2 = "select to_char(t.riqi,'yyyy-mm'),sum(t.sfje),sum(t.cbje) from Tkd t where t.jystate=1 "+wheres;
