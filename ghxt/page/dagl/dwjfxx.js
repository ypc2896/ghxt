...
Ext.onReady(function(){
	...
	//查询表单
	   ...
	   var qbStore = new Ext.data.JsonStore({
		url: 'aaacsxx_findAaacsxxComb.do',
	    root: 'root',
	    totalProperty: 'total',
	    fields: ['value','text'],
	    listeners:{
	    			load:function(s){
	    				var r = new qbStore.recordType({value:'',text:'全部'});
    					qbStore.insert(0,r);
	    			}
	    		  }
		});     
...
//							width:80
	...
	var DwjfxxObj = [
			{name:'aae061',type:'string'},    
			{name:'aab001',type:'string'},    
			{name:'aae062',type:'string'},    
			{name:'aae063',type:'float'},
			{name:'aae064',type:'string'},    
			{name:'aae065',type:'string'},    
			{name:'aae072',type:'float'},
			{name:'aab003',type:'string'},    
			{name:'aae073',type:'float'},
			{name:'aae074',type:'float'},
			{name:'aae075',type:'float'},
			{name:'aae076',type:'float'},
			{name:'aae077',type:'float'},
			{name:'aae078',type:'float'},
			{name:'aae079',type:'float'},
			{name:'aae080',type:'string'},    
			{name:'aae081',type:'float'},
			{name:'aae082',type:'float'},
			{name:'aae083',type:'string'},    
			{name:'aab005',type:'string'},    
			{name:'aab006',type:'string'},    
			{name:'aab007',type:'string'},    
			{name:'aab011',type:'string'},    
			{name:'aab012',type:'string'},    
			{name:'aae084',type:'string'},    
			{name:'aae085',type:'string'},    
			{name:'aae086',type:'string'},    
			{name:'aae087',type:'string'},    
			{name:'aae088',type:'string'},    
			{name:'aae089',type:'string'},    
			{name:'aae090',type:'string'},    
			{name:'aae091',type:'string'},    
			{name:'aae092',type:'string'},    
			{name:'aae093',type:'string'},    
			{name:'aae094',type:'string'},    
			{name:'aae095',type:'string'},    
			{name:'aae096',type:'string'},    
			{name:'aae097',type:'string'}    
	...
	var store = new Ext.data.JsonStore({
		...
		if (f.isValid()) {
			...
			//store.load({params:params});
	    a.baseParams={start:v_start, limit:v_limit,gwgl:v_gwgl,aae084:v_cybz,startdate:params.startdate,enddate:params.enddate,aae062:params.aae062};//aae062为期别，期别查询与缴费状态树邦定。
	    }
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
	            ...
	            {header: '期别', width: 40,sortable:true, align:'center', dataIndex: 'aae062'},
	            {header: '缴费ID', width: 50,sortable:true, align:'center', dataIndex: 'aae061'},
	            {header: '单位编号', width: 60,sortable:true,  align:'center', dataIndex: 'aab001'},
	            {id:'aab003',header:'单位名称',width:150,sortable:true, align:'center',dataIndex:'aab003'},
	            {header: '缴费人数', width: 60, sortable:true, align:'center',dataIndex: 'aae072'},
	            {header:'标准',width:40,sortable:true, align:'center',dataIndex:'aae073'},
	            {header: '应缴金额', width: 60, sortable:true, align:'center',dataIndex: 'aae063'},
				
				{header:'在职人数',width:60,sortable:true, align:'center',dataIndex:'aae075'},   
				{header:'内退人数',width:60,sortable:true, align:'center',dataIndex:'aae076'},				
				{header:'人员比例',width:60,sortable:true, align:'center',dataIndex:'aae074'},   
				{header:'医保人数',width:60,sortable:true, align:'center',dataIndex:'aae077'},   
				//{ header:'发文日期',width:70,sortable:true, align:'center',renderer : Ext.util.Format.dateRenderer('Y-m-d'), dataIndex:'gw006'},   
				{ header:'未参医保',width:60,align:'center',dataIndex:'aae078'},   
				{ header:'变动人数',width:60,align:'center',dataIndex:'aae079'},   
				{ header:'变动原因',width:60,align:'center',dataIndex:'aae080'},   
				{ header:'农民工',width:50,align:'center',dataIndex:'aae081'},   
				...
				{ header:'非农民工',width:60,sortable:true, align:'center',dataIndex:'aae082'},  
				{ header:'首参',width:50,sortable:true, align:'center',dataIndex:'aae083',
									renderer : function(v) {return v==0?"是":"否";}},
				{ header:'缴费审批',width:60,align:'center',sortable:true,dataIndex:'aae084',
				                 renderer: function(v){
				                 	var str="";
				                 	switch(v){ 
				                 	...
				                 	case '4': str='待审批';  break;
				                 	case '5': str='已审批';  break;
				                 	case '6': str='待收款';  break;
				                 	case '7': str='已回款';  break;
				                 	case '8': str='已归档';  break;
				                 	}
			                 		return str;
				                 	}
				}
...
                 }  
        ...
        listeners:{
        	rowclick:function(a,b){
        		...
        		//ycmxstore.removeAll();
         		//alert(b);
         		  ycmxstore.load({params:{aae061:gwrecord1.get("aae061"),aae005:gwrecord1.get("aae062"),start:v_start, limit:v_limit}});
...
                    leaf:false, 
					expanded: true, 
                    children:[  
                     {id:'0',text:'0_未上报',leaf:true },
					 {id:'1',text:'1_待审核',leaf:true},
					 {id:'2',text:'2_审核中',leaf:true},
					 {id:'3',text:'3_已审核',leaf:true},
					 {id:'4',text:'4_待审批',leaf:true},
					 {id:'5',text:'5_已审批',leaf:true},
					 {id:'6',text:'6_待收款',leaf:true},
					 {id:'7',text:'7_已收款',leaf:true},
					 {id:'8',text:'8_已归档',leaf:true}   
                    ]  
                });  
  
...
                    width:125,
                    listeners:{        	
        		        ...
        		        //更新数据
        		        store.load({params:{start:v_start, limit:v_limit,aae084:v_cybz}});
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
			   //smm,
			    ...
			    { header:'缴费ID',width:60,align:'center',dataIndex:'aae061'},
			    { header:'单位编号',width:60,align:'center',dataIndex:'aab001'},
			    { id:'aab003', header:'单位名称',width:60,align:'center',dataIndex:'aab003'},
	            { header: '序号', width: 60,sortable:true, align:'center', dataIndex: 'aae010'},
